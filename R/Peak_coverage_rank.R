...
#'
#' Peak_coverage_rank() function ranks (ascending) the coverage proportion per peak of a group across groups 
...
Peak_coverage_rank <- function(tsv_file_df, group_by_column,  group, background_group, bw_table){
  ...
  final_coverage <- multiBigwig_summary(data_table = bw_table, summary_type = "mean", data_frame_of_regions = bed_regions, parallel = TRUE  )
  ...
  #final_coverage$coverage_prop <- final_coverage[,grep(paste0("^",group,"$",collapse = ""), colnames(final_coverage), fixed=TRUE)]/final_coverage$coverage_sum
  setDT(final_coverage)[order(-coverage_prop), peak_cov_rank := rleid(coverage_prop)]
  ###colnames(final_coverage) == group
